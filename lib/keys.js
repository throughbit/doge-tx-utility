  ...
  try{
    return new Promise((resolve,reject)=>{
      ...
      async_looper.go(sets,(set,report)=>{    
        ...
        const pk = new doge.PrivateKey();
        const pubk = pk.toPublicKey();
        const addr = pubk.toAddress(doge.Networks.livenet);
        ...
        //console.log(isMultiSig);
          if (isMultiSig){
           ...
           //console.log(pubkey_set);
            req_options.build("interface",params,"POST","/create_multisig")
            .then((options)=>{
              request(options,(error,response,body)=>{
                ...
                if(body===undefined){
                  reject("Undefined body from request to /create_multisig");
                }
                if(body.status){
                  let multisig_form = {
                    keys: key_sets,
                  ...
                  let responso = res_fmt.create(true,multisig_form);
                  //console.log(body.message);
                  resolve(responso);
                }
                else if(!body.status){
                  console.log("Error in /create_multisig request to node interface.");
                  reject(body.message);
                }
              });
            })
